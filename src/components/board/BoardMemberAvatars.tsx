...
export default function BoardMemberAvatars({
  participants,
  onlineUserIds = [],
  onOpenShare,
}: BoardMemberAvatarsProps) {
  const visible = participants.slice(0, MAX_VISIBLE);
  const overflow = participants.length - MAX_VISIBLE;

  return (
    <div className="inline-flex h-9 items-center gap-1.5 rounded-full bg-black/14 px-1.5 pr-1.5 text-sm font-medium text-white/88 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-sm">
      <div className="flex items-center -space-x-2">
        {visible.map((participant) => (
          <div
            key={participant.userId}
            className="relative flex size-7 items-center justify-center rounded-full border border-white/45 bg-white/92 text-[10px] font-semibold text-slate-700 shadow-sm"
